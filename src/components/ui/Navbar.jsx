...
export const Navbar = () => {
  ...
  const { isAuthenticated, isStudent, name } = useSelector(
    (state) => state.auth
  );
  ...
  const user = (
    <SplitButton
      label={nameItem}
      icon="pi pi-user"
      className={`${isStudent ? "p-button-warning" : "p-button-info"}`}
      model={itemsUser}
    />
  );
    ...
    >
      <Menubar
        ...
        model={
          isAuthenticated
            ? isStudent
              ? items.concat(itemsStudent)
              : items.concat(itemsProfessor)
            : items
        }
        start={start}
        end={isAuthenticated ? user : access}
      />
    </motion.div>
  );
};
